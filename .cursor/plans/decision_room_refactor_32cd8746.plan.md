...
todos:
  ...
  - id: phase0
    ...
    status: completed
  - id: phase1
    content: Implement Phase 1 Chat Room (sequential loop; mutex only if concurrency is introduced) in orchestrator/phases/phase1_chatroom.py
    status: completed
  - id: phase2
...
**Phase 1 locking rule**: For strict sequential turn-taking (agent A then B), no mutex is required. Add an `asyncio.Lock` only if we later allow concurrent/speculative turn generation that could write to `state.debate_transcript` in parallel.
...
class SystemOrchestrator:
    ...
    async def run(self, user_prompt: str) -> DecisionRoomState:
        ...
        # Phase 1: Debate loop (sequential; no lock required unless concurrent writes are introduced)
    ...
    async def _phase1_debate(self, state: DecisionRoomState) -> DecisionRoomState:
        ...  # Loop 3-4 turns, each agent speaks in order
